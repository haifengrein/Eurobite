# EuroBite 2026 - Frontend Refactoring Plan (React SPA, v2)

> 说明：本文件整合了前台移动端（Client）和后台管理端（Admin）的所有前端开发任务。
> 保持与 `docs/api-contract-frontend.md` 中定义的 API 契约一致。
> 技术栈：React 18+, TypeScript, Vite, Tailwind CSS, Zustand, Axios. 
> UI 库：Client 端使用 Shadcn/UI (可选) / Tailwind; Admin 端使用 Ant Design.

---

# Part 1: Client Application (前台移动端)
**目标：** 构建面向消费者的移动端 Web 应用。

## Phase 1: Scaffolding & Infrastructure
**目标：建立专业的前端工程基线。**

- [x] **Task 1.1: 初始化工程**
  - [x] 使用 `npm create vite@latest` 创建 `eurobite-web`（React + TypeScript 模板）/ 等价结构。  
  - [x] 集成 Tailwind CSS + PostCSS。  
  - [x] 配置 ESLint + Prettier（可参考 Airbnb 或自定义规则）。  
  - [x] 在 `tsconfig.json` 和 `vite.config.ts` 中配置路径别名（例如 `@/components`, `@/api`, `@/store`）。  

- [x] **Task 1.2: 核心依赖**
  - [x] 安装 `axios`（HTTP）、`react-router-dom`（路由）、`zustand`（状态管理）、`clsx` / `tailwind-merge`（样式工具）。  
  - [ ] 可选：集成 Shadcn/UI，用于基础组件（Button/Input/Card/Toast）。  

- [x] **Task 1.3: 环境配置**
  - [x] 使用 `.env` / `.env.example` 配置 `VITE_API_BASE_URL`，支持本地与生产环境。  
  - [ ] 在开发与生产环境中使用不同的后端地址（待根据部署方案补完）。  

- [x] **Task 1.4: 网络层（API Client）**
  - [x] 创建 `src/lib/http.ts` 自定义 Axios 实例。  
  - [x] 请求拦截器：自动附加 `Authorization: Bearer <token>` 头。  
  - [x] 响应拦截器：将错误回传上层，由各页面结合全局 Toast 处理（401 跳转逻辑按需在后续补充）。  

---

## Phase 2: User Authentication（用户认证）
**目标：与后端用户认证 API（见 `/api/auth/user/login`, `/api/user/me`）对接。**

- [x] **Task 2.1: 数据模型与 Store**
  - [x] 定义 `User` 类型（`id`, `name`, `phone`, `token` 等）。  
  - [x] 使用 Zustand 创建 `useUserStore`：包含 `user`, `setSession`, `clearSession` 等。  
  - [x] 将 token 持久化到 `localStorage`。  

- [x] **Task 2.2: 登录页面**
  - [x] UI：手机号输入、验证码输入、提交按钮。  
  - [x] 调用 `POST /api/auth/user/login`，成功后保存 Token 与用户信息并跳转到首页/来源页。  
  - [ ] 表单校验：可使用 React Hook Form + Zod（手机号格式、必填）。  

- [x] **Task 2.3: 受保护路由**
  - [x] 实现 `RequireAuth` 组件：若未登录，重定向到登录页。  
  - [x] 对订单列表、个人中心、结算、地址管理、订单详情等页面启用路由保护。  

---

## Phase 3: Browsing Menu（点餐首页）
**目标：实现分类 + 菜品列表的主流程。**

- [x] **Task 3.1: 布局与导航**
  - [x] 创建 `MainLayout`，包含顶部标题与底部 Tab Bar（首页、订单、个人）。  

- [x] **Task 3.2: 分类侧边栏**
  - [x] 调用 `GET /api/menu/categories` 获取分类列表。  
  - [x] 实现垂直侧边栏，点击分类过滤右侧菜品列表。  

- [x] **Task 3.3: 菜品列表**
  - [x] 调用 `GET /api/menu/dishes?categoryId=...` 获取菜品。  
  - [x] 渲染菜品卡片（图片、名称、价格、加购按钮）。  
  - [ ] 可选优化：虚拟列表或图片懒加载。  

---

## Phase 4: Shopping Cart（购物车）
**目标：前端购物车状态与后端 `/api/cart` 同步。**

- [x] **Task 4.1: Cart Store**
  - [x] 使用 Zustand 创建 `useCartStore`，包含 `items`, `addDish`, `decreaseItem`, `removeItem`, `clearAll` 等。  
  - [x] 与后端 API 对应：
    - [x] `GET /api/cart`：初始化购物车状态。  
    - [x] `POST /api/cart/items`：添加/数量 +1。  
    - [x] `PATCH /api/cart/items/{itemId}/decrease`：数量 -1。  
    - [x] `DELETE /api/cart/items/{itemId}` / `DELETE /api/cart`。  

- [x] **Task 4.2: 底部购物车条**
  - [x] 在首页底部显示总价与数量，点击“去结算”跳转到结算页。  
  - [x] 使用乐观更新提升交互流畅度。  

---

## Phase 5: Checkout & Address Management（下单与地址管理）
**目标：完成从购物车到下单的完整流程。**

- [x] **Task 5.1: 地址管理页面**
  - [x] 使用 `/api/address-book` 系列接口：
    - [x] `GET /api/address-book`：地址列表。  
    - [x] `POST /api/address-book`：新增地址。  
    - [x] `PUT /api/address-book/{id}`：编辑地址。  
    - [x] `DELETE /api/address-book/{id}`：删除地址。  
  - [x] 提供地址新增/编辑表单（基础表单校验）。  

- [x] **Task 5.2: 结算页**
  - [x] 展示订单确认信息（来自购物车）。  
  - [x] 选择收货地址（从 `/api/address-book` 获取）。  
  - [x] 提交订单：调用 `POST /api/orders`（请求体包含 `addressId`, `remark` 等）。  
  - [x] 成功后通过 Toast 提示并跳转订单列表。  

---

## Phase 6: Order History（订单中心）
**目标：让用户查看历史订单与详情。**

- [x] **Task 6.1: 订单列表页**
  - [x] 调用 `GET /api/orders/me?page=&size=&status=` 获取当前用户订单列表。  
  - [x] 按状态进行标签筛选（全部、进行中、已完成等）。  

- [x] **Task 6.2: 订单详情页**
  - [x] 调用 `GET /api/orders/{orderId}` 获取详情。  
  - [x] 展示菜品列表、金额、下单时间、状态、收货信息。  

---

## Phase 7: Polish & Build（打磨与构建部署）

- [ ] **Task 7.1: 性能与体验优化**
  - [x] 路由懒加载（Code Splitting）。  
  - [ ] 基础 PWA 支持（Manifest + Service Worker）以支持“添加到主屏幕”（可选）。  
  - [x] 简单 Skeleton/Loading 状态优化（首页菜品列表、订单列表骨架屏）。  

- [ ] **Task 7.2: 构建与集成**
  - [ ] 使用 `npm run build` 生成静态资源，并在文档中说明构建产物位置。  
  - [x] 为前端创建独立 Dockerfile（Node 构建 + Nginx 运行）。  
  - [x] 在 `eurobite/docker-compose.yml` 中新增 `frontend` 服务，支持一键起前端容器。  
  - [ ] 针对实际后端部署方案，选择以下方式之一完成集成：  
    - 作为静态资源拷贝到后端 `src/main/resources/static` 下，由 Spring Boot 提供服务。  
    - 独立 Docker 镜像（Nginx + 前端静态文件），通过反向代理与后端交互（当前已具备镜像构建与启动能力，反向代理配置待落地）。  

---

# Part 2: Admin Interface (后台管理端)
**目标：** 内部员工使用的管理后台。
**注意：** 前端项目结构已调整，建议创建独立的 `eurobite-admin` 项目或在 `frontend` 目录下建立 `admin` 子模块。
